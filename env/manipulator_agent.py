...
from env.mujoco.mujoco_utils import pr2t,r2w,rpy2r,trim_scale,meters2xyz,compute_view_params, get_interp_const_vel_traj, sample_xyzs, r2rpy

class ManipulatorAgent(object):
    def __init__(self,rel_xml_path=None,USE_MUJOCO_VIEWER=False,VERBOSE=True, MODE="window"):
        """
        ...
        if self.rel_xml_path is not None:
            self._parse_xml()
        self.obj_name_lst = [body_name for body_name in self.body_names
    ...
    def reset(self, q=None):
        ...
        if q is None:
            ...
            self.data.qpos[self.idxs_forward] = q 
            mujoco.mj_resetData(self.model,self.data)
            mujoco.mj_forward(self.model,self.data)
            self.forward(q, self.idxs_forward)
        self.tick        = 0
        self.render_tick = 0
    ...
    def render(self,render_every=1, p_ego=np.array([0, 0, 0]), p_trgt=np.array([1.0, 1.0, 1.0])):
        """
            Render
        """
        ...
        if self.MODE == "window":
            ...
            rgb, depth = self.grab_rgb_depth_img_offscreen(p_ego=p_ego,p_trgt=p_trgt)
            self.render_tick = self.render_tick + 1
            return rgb, depth
    ...
    def grab_rgbd_image(self):
        """
            Grab the rendered depth image
        """
        # Grab image
        mujoco.mjr_render(self.viewer.viewport, self.viewer.scn, self.viewer.ctx)
        img = np.zeros((self.viewer.viewport.height,self.viewer.viewport.width,3),dtype=np.uint8)
        depth_img = np.zeros((self.viewer.viewport.height,self.viewer.viewport.width, 1), dtype=np.float32)
        mujoco.mjr_readPixels(img, depth_img, self.viewer.viewport, self.viewer.ctx) # this makes a little delay
        img = np.flipud(img) # flip image
    ...
    def solve_ik_w_cc(self,body_name,p_trgt,R_trgt,IK_P,IK_R,q_init,idxs_forward, idxs_jacobian,
                 RESET=False,DO_RENDER=False,render_every=1,th=1*np.pi/180.0,err_th=1e-6,w_weight=1.0, collisioin_check=False):
        """
            Solve IK
        """
        if RESET:
        ...
        while True:
            ...
            self.forward(q=q,joint_idxs=idxs_forward)
            if collisioin_check:
                exc_prefix = ['obj_cylinder_01', 'obj_cylinder_02', 'obj_cylinder_03', 'obj_cylinder_04', 'obj_box_01']
                p_contacts,f_contacts,geom1s,geom2s,body1s,body2s = self.get_contact_info(must_exclude_prefix=exc_prefix)
                
                body1s_ = [obj_ for obj_ in body1s if obj_ not in ["rg2_gripper_finger1_finger_tip_link","rg2_gripper_finger2_finger_tip_link", "front_object_table"]]
                body2s_ = [obj_ for obj_ in body2s if obj_ not in ["rg2_gripper_finger1_finger_tip_link","rg2_gripper_finger2_finger_tip_link", "front_object_table"]]
                # print('body1s_', body1s_)
                # print('body2s_', body2s_)
                if len(body1s_)>=1: 
                    FLAG = True 
            # Terminate condition
            err_norm = np.linalg.norm(err)
    ...
    def grab_rgb_depth_img_offscreen(self, p_ego=None,p_trgt=None):
        """
        ...
        if (p_ego is not None) and (p_trgt is not None):
            ...
            print("There's no specific view pose.")
            self.update_viewer(azimuth=0,distance=1.0,
                               elevation=-60,lookat=[1.06, 0., 0.72])
    ...
    def move_sweep_traj(self, ys, xs, vel=np.radians(10), HZ=500):
        init_sweep_pose = [-0.73418, -1.08485, 2.7836, -1.699, 0.8366, 0]
        self.forward(q=init_sweep_pose,joint_idxs=self.idxs_forward)
        q_list  = []
        for idxs, (x, y) in enumerate(zip(xs, ys)):
            if idxs== 0:
                q_ik = self.solve_ik(
                    body_name='tcp_link', p_trgt=np.array([x[0], y[0], 0.85]), R_trgt=rpy2r(np.radians([-180,0,90])), IK_P=True, IK_R=True,
                    q_init=init_sweep_pose, idxs_forward=self.idxs_forward, idxs_jacobian=self.idxs_jacobian,
                    err_th=1e-4, w_weight=0.2)
            else:
                if idxs<2:
                    q_ik = self.solve_ik(
                        body_name='tcp_link', p_trgt=np.array([x[0], y[0], 0.85]), R_trgt=rpy2r(np.radians([-180,0,90])), IK_P=True, IK_R=True,
                        q_init=q_ik, idxs_forward=self.idxs_forward, idxs_jacobian=self.idxs_jacobian,
                        err_th=1e-4, w_weight=0.2) 
                else:    
                    q_ik = self.solve_ik(
                        body_name='tcp_link', p_trgt=np.array([x[0], y[0], 0.805]), R_trgt=rpy2r(np.radians([-180,0,90])), IK_P=True, IK_R=True,
                        q_init=q_ik, idxs_forward=self.idxs_forward, idxs_jacobian=self.idxs_jacobian,
                        err_th=1e-4, w_weight=0.2)
            q_list.append(q_ik)
        times,q_traj = get_interp_const_vel_traj(np.array(q_list), vel=vel, HZ=HZ)
        return times, q_traj

    def move_place_traj(self, xs, ys, zs, vel=np.radians(20), HZ=500):
        init_place_pose = [0, -1.57, 1.57, 1.57, 0, 0]
        self.forward(q=init_place_pose,joint_idxs=self.idxs_forward)
        q_list = []
        for idxs, (x, y, z) in enumerate(zip(xs, ys, zs)):
            if idxs== 0:
                q_ik = self.solve_ik(
                    body_name='tcp_link', p_trgt=np.array([x[0], y[0], z[0]]), R_trgt=rpy2r(np.radians([90,180,-90])), IK_P=True, IK_R=True,
                    q_init=init_place_pose, idxs_forward=self.idxs_forward, idxs_jacobian=self.idxs_jacobian,
                    err_th=1e-4, w_weight=0.2)
            else:
                q_ik = self.solve_ik(
                    body_name='tcp_link', p_trgt=np.array([x[0], y[0], z[0]]), R_trgt=rpy2r(np.radians([90,180,-90])), IK_P=True, IK_R=True,
                    q_init=q_ik, idxs_forward=self.idxs_forward, idxs_jacobian=self.idxs_jacobian,
                    err_th=1e-4, w_weight=0.2) 
            q_ik_w_gripper = np.concatenate((q_ik, np.array([1])))
            q_list.append(q_ik_w_gripper)
        times,q_traj = get_interp_const_vel_traj(np.array(q_list), vel=vel, HZ=HZ)
        return times, q_traj

    def move_reach_traj(self, xs, ys, zs, vel=np.radians(20), HZ=500):
        init_reach_pose = np.array([0, -1.57, 0, -1.57, -3.14, 3.14])
        self.forward(q=init_reach_pose,joint_idxs=self.idxs_forward)
        q_list = []
        for idxs, (x, y, z) in enumerate(zip(xs, ys, zs)):
            if idxs== 0:
                q_ik = self.solve_ik(
                    body_name='tcp_link', p_trgt=np.array([x[0], y[0], z[0]]), R_trgt=rpy2r(np.radians([90,180,-90])), IK_P=True, IK_R=False,
                    q_init=init_reach_pose, idxs_forward=self.idxs_forward, idxs_jacobian=self.idxs_jacobian,
                    err_th=1e-4, w_weight=0.2)
            else:
                q_ik = self.solve_ik(
                    body_name='tcp_link', p_trgt=np.array([x[0], y[0], z[0]]), R_trgt=rpy2r(np.radians([90,180,-90])), IK_P=True, IK_R=False,
    ...
    def move_sweep_traj2(self, ys, xs, vel=np.radians(10), HZ=500):
        init_sweep_pose = [-0.73418, -1.08485, 2.7836, -1.699, 0.8366, 0]
        self.forward(q=init_sweep_pose,joint_idxs=self.idxs_forward)
        q_list  = []
        for idxs, (x, y) in enumerate(zip(xs, ys)):
            if idxs== 0:
                q_ik = self.solve_ik(
                    body_name='tcp_link', p_trgt=np.array([x, y, 0.85]), R_trgt=rpy2r(np.radians([-180,0,90])), IK_P=True, IK_R=True,
                    q_init=init_sweep_pose, idxs_forward=self.idxs_forward, idxs_jacobian=self.idxs_jacobian,
                    err_th=1e-4, w_weight=0.2)
            else:
                if idxs<2:
                    ...
                    q_ik = self.solve_ik(
                        body_name='tcp_link', p_trgt=np.array([x, y, 0.805]), R_trgt=rpy2r(np.radians([-180,0,90])), IK_P=True, IK_R=True,
                        q_init=q_ik, idxs_forward=self.idxs_forward, idxs_jacobian=self.idxs_jacobian,
                        err_th=1e-4, w_weight=0.2)
            q_list.append(q_ik)
        times,q_traj = get_interp_const_vel_traj(np.array(q_list), vel=vel, HZ=HZ)
