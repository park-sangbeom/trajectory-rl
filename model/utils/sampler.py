...
def laqdpp(xs_total, n_select, qs_total=None, k_gain=100, q_scale=0.01):
    """
    Locally Approximated Quality-based DPP"
    """
    qs_total=qs_total*q_scale
    n_total= xs_total.shape[0]
    sum_K  = np.zeros((1,n_total))
    dim    = xs_total.shape[1]
    # D      = cdist(xs_total,xs_total,'euclidean')
    D      = distance_matrix(xs_total,xs_total,p=2)
    SCORE_MAT = np.ones_like(D)
    if qs_total is not None:
        np.fill_diagonal(SCORE_MAT,qs_total)
    K_MAT    = SCORE_MAT*np.exp(-k_gain*(D))
    xs_selected   = np.zeros(shape=(n_select, dim))
    sel_idx_total = np.zeros(n_select)
    sel_idx_temp  = np.zeros(n_select)
    remain_idx    = np.arange(n_total)

    for n_idx in range(n_select):
        if n_idx==0:
            sel_idx = np.random.randint(n_select)
        else: 
            curr_K = K_MAT[int(sel_idx_total[n_idx-1]),:]
            curr_score =qs_total[int(sel_idx_total[n_idx-1])]
            if curr_score == 0:
                curr_score = 1e-5
            sum_K += (curr_K)**2/curr_score
            k_val = sum_K[:, remain_idx]
            approximated_k = qs_total[remain_idx] - k_val 
            sel_idx = np.argmax(approximated_k)
        sel_idx_temp = remain_idx[sel_idx]
        sel_idx_total[n_idx]=int(sel_idx_temp)
        remain_idx = np.delete(remain_idx, sel_idx).reshape(-1)
        xs_selected[n_idx, :] = xs_total[sel_idx_temp,:] # append a selected point
    xs_selected=xs_selected[1:,:]
    return xs_selected, sel_idx_total.astype(int)  